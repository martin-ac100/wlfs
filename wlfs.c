...
extern wlfs_config_t wlfs_config;

int wlfs_find_rec() {
	HEADER_LEN = WRITE_ALIGN(wlfs_config, sizeof(wlfs_header_t));
	...
	while ( cursor_offset+sizeof(wlfs_header_t) <= wlfs_config.end ) {
		...
		cursor_offset = SECTOR_ALIGN(wlfs_config, cursor_offset + HEADER_LEN + cursor.len);
	}
